...
function searchMember(isScan) {
 ...
 function fetchBookDetails(bookID, memberID) {
    ...
    console.log("Book ID: ", bookID);  // Log Book ID
    console.log("Member ID: ", memberID);  // Log Member ID
...
function validateBookAvailability() {
    ...
    var memberID = document.getElementById('memberID').value;  // Make sure this ID matches your input field for member ID
    fetchBookDetails(bookID, memberID);
}
   
function validateBookAvailability() {
    var isbn = document.getElementById('ISBN').value;
    var memberID = document.getElementById('memberID').value; // Ensure you have an input for member ID
...
function returnBook(event) {
    ...
    .then(data => {
        if (data.success) {
            togglePopup('returnSuccessPopup', true);  // Show success popup
            document.getElementById('issueForm').reset();  // Reset form
        } else {
...
function displayReturns() {
    var memberID = document.getElementById('memberID').value; // Corrected to use getElementById
    ...
    .then(data => {
        ...
        data.forEach(issue => {
            ...
            row.insertCell(4).textContent = issue.ReturnDate || 'N/A'; // Assuming there is a 'returnDate' field in your data
