...
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
        addQuickLinksAsUserMessages();
    }, 1000);  // Run after the greeting is displayed
});

function addQuickLinksAsUserMessages() {
    const quickLinks = ['Most Popular Book', 'Popular Box', 'Book Genres', 'Library Opening Hours'];
    quickLinks.forEach(link => {
        const messageLi = document.createElement('li');
        messageLi.classList.add('chat', 'user-initiated');
        messageLi.innerHTML = `<button class="recommend-btn">${link}</button>`;
        messageLi.addEventListener('click', function() {
            userMessage = link;
            handleChat(); // Process the selection
        });
...
function getQueryTypeFromIntent(intent) {
    switch (intent.name) {
        ...
        default:
            return null;  // Handle unrecognized intents
    }
}
